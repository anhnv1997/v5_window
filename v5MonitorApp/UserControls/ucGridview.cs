...
namespace v5MonitorApp.UserControls
{
    public partial class ucGridview<T> : UserControl where T : Control
    {
        ...
        public void PollingStart()
        {
            foreach (Control child in children)
            {
                if (child is IDeviceMonitor)
                {
                    IDeviceMonitor monitor = (IDeviceMonitor)child;
                    monitor.onMonitorDeviceNewMessageEvent += Monitor_onMonitorDeviceNewMessageEvent;
                    monitor.PollingStart();
                }
            }
        }
        ...
        private void Monitor_onMonitorDeviceNewMessageEvent(object sender, DeviceMessageEventArgs deviceMessageEventArgs)
        {
            this.onMonitorDeviceNewMessageEvent?.Invoke(sender, deviceMessageEventArgs);
        }
    }
}
