...
namespace iParkingv5.LedDisplay.KztekDevices.ParkingLed
{
    public class P762RGY : GeneralIpLedController
    {
        public P762RGY(string ipAddress, int port, EmModuleType modultType, string username = "", string password = "") : base(ipAddress, port, modultType, username, password)
        {
        }
        ...
        public override bool ChangeDisplay(EmScreenType screen, Dictionary<int, LineConfig> datas, EmDisplayMode displayMode, List<MergeSetting> mergeSettings, List<ScrollSetting> scrollSettings, bool isScroll = false)
        {
            if (isScroll)
            {
                ctsScroll = new System.Threading.CancellationTokenSource();
            }
