    ...
    public partial class ucLaneOut : UserControl, iLane, IDisposable
    {
        ...
        private async void ExcecuteVIPProcess(Identity identity)
        {
            ...
            if (lastEvent == null)
            {
                ...
                var data = await AppData.ApiServer.GetEventOuts("", startTime, endTime, "", "", "", "", 0, 1);
                if (data.data.Count > 0)
                {
                    string plateNumber = data.data[0].plateNumber;
                    string id = data.data[0].id;
                    string message = $"Bạn có xác nhận sử dụng thẻ  {identity.Name} cho phương tiện {plateNumber}";
                    bool isUpdate = new frmConfirmSendInvoice(message).ShowDialog() == DialogResult.OK;
                    if (isUpdate)
                    {
                        //Cập nhật note lý do không thu tiền
                        string currentNote = data.data[0].thirdpartynote ?? "";
                        string[] currentNoteArray = Newtonsoft.Json.JsonConvert.DeserializeObject<List<string>>(currentNote)!.ToArray();// note.Split(";");
                        List<string> temp = new List<string>();
                        for (int i = 0; i < 2; i++)
                        {
                            if (currentNoteArray.Length > i)
                            {
                                temp.Add(currentNoteArray[i]);
                            }
                        }
                        string note1 = currentNoteArray.Length > 1 ? currentNoteArray[0] : "";
                        string note2 = identity.Description;
                        ...
                        await KzParkingv5ApiHelper.DisActive(identity.Id);
                    }
                }
                else
                {
